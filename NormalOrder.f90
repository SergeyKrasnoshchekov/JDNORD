...
!      This project consisting from 2 files was compiled with Intel Fortran IFORT with
!      an essential option: Project Properties (Alt-Enter) > 
!      Configuration Properties > Fortran > Data > Default Real KIND = 16

!-----------------------------------------------------------------------------------------------
!  MODULE: NORMAL
!
!  MODULE: PROBE_REDUCEJ                                                       EDIT: 12 Feb 2023
!  MODULE: REDUCEJ                                                             EDIT: 12 Feb 2023
!  MODULE: ROTOPKET                                                            EDIT: 12 Feb 2023
!
!  MODULE: PROBE_DREDUCE                                                       EDIT: 12 Feb 2023
!  MODULE: DREDUCE                                                             EDIT: 12 Feb 2023
!  MODULE: WIGOPKET                                                            EDIT: 12 Feb 2023
!
!  MODULE: BINOM_TABLE                                                         EDIT: 12 Feb 2023
!  MODULE: STIRL_TABLE                                                         EDIT: 12 Feb 2023
!-----------------------------------------------------------------------------------------------
...
!        NTMP -- Auxiliary tempopery binary file(s);
...
!----    Logical unit numbers: textual output to the listing file, standard input/output
 ...
 1000 FORMAT (/'Demonstration and verification of normal ordering of angular momentum'/  &
     ...
     &   'Partial results are shown on the screen, full output can be found in Listing.log')
      ...
      MAXPOW = 6
      JMAX = 9
      CALL PROBE_REDUCEJ (MAXPOW, JMAX, IERR)
...
!
!-----------------------------------------------------------------------------------------------
      SUBROUTINE PROBE_REDUCEJ (MAXPOW,JMAX,IERR)
...
!----          Reduce J~6 to a linear combination of terms: SUM(j=1,N) c(j) J~3(j)
            ...
            JPOW(2) = B;  JPOW(5) = E  
            JPOW(3) = C;  JPOW(6) = F  
            CALL REDUCEJ (JPOW,MAXDIM,NTERM,JOPRED,CONRED,IRET)
            IF (IRET /= 0) GO TO 500
...
!----             Skip second pass if no errors encountered
            IF (IEQUAL == 1 .AND. DELTA < TOLDEL) THEN
               EXIT
            ELSE
               NERR = NERR + 1
               JERR = JERR + 1
 ...
 1210 FORMAT ('Testing JJJJJJ for fixed J is accomplished, the number of dubious cases = ',I0)
 ...
 3200 FORMAT (/'#### PROBE_REDUCEJ ####'/'The test was successful.')
 3210 FORMAT (/'#### PROBE_REDUCEJ ####'/'The test FAILED.')
      END
...
!               equat to the maximum number of terms before collecting like terms.
!
!  OUTPUT:
!     NTERM  -- The number of generated terms;
!     JOUT   -- Array of triples (k,l,m) of powers of normilized rotational operators;
!     COEF   -- Integer(8): Coefficients of normalized terms, cannot be limited to Integer(4);
!     IERR   -- Return code, = 0 for success; and 
!               = 1 if the the number of generated terms exceeds MAXDIM.
!
!-----------------------------------------------------------------------------------------------
      SUBROUTINE REDUCEJ (JINP,MAXDIM,NTERM,JOUT,COEF,IERR)
      IMPLICIT REAL (16) (A-H,O-Z), INTEGER (4) (I-N)
...
!        J(-) | J, K > = \/ (J+K)(J-K+1)  | J, K-1 >
      DO I = 1, JOPROT(3)
         ...
         IF (K - 1 < -J) GO TO 400
         K = K - 1
         COEF = COEF * SQRT (REAL (NUM, KIND = 16))
      ENDDO
...
!-----------------------------------------------------------------------------------------------
      ...
      INTEGER (1) :: WIGOPER(-1:+1),JOPER(3)                                                        !  (4)
      INTEGER (4) :: KETROT(2),KETJJJ(2)
      INTEGER (4) :: KETWIG(2,-1:+1)  !  |J'(j),K'(j)>, j = -1, 0, +1
      REAL (16) :: WCOEF(-1:+1)

      INTEGER (1), ALLOCATABLE :: JOPERARR(:,:),WOPERARR(:,:)                                       !  (4)
...
!----       INPUT:  INT*4 WIGOPER(-1:+1), INT*4 KETROT(2)
!----       OUTPUT: INT*4 KETWIG(2,-1:+1), REAL*16 WCOEF
...
!-----------------------------------------------------------------------------------------------
      OPEN (UNIT = NTMP, FORM = 'UNFORMATTED', STATUS = 'SCRATCH', BUFFERED = 'YES')
...
!-----------------------------------------------------------------------------------------------
      ...
      ENDIF
      IF (LSM > 1) THEN
         PAUSE 'DREDUCE: Total power of D-function > 1'
         STOP
      ENDIF
...
!----    Reduce (Jz - 1)^a to normal form
  ...
  110 CONTINUE
      ...
      IF (IPOWB > 0) THEN
         NTERM = NTERM + 1
         DCOEF(NTERM) = - SQRT (2.0q0) * REAL (IPOWB, KIND = 16)
...
!----    Reduce (Jz + 1)^a to normal form
      IF (IPOWB > 1) THEN
         IPROD = IPOWB * (IPOWB - 1)
         DO 120 K = 0, IPOWA
            COEF = REAL (IPROD, KIND = 16) * REAL (BINOM (IPOWA, K), KIND = 16)
            NTERM = NTERM + 1
            DCOEF(NTERM) = COEF
...
!----    Reduce (Jz - 1)^a to normal form
      ...
      DO 210 K = 0, IPOWA
         ...
         NTERM = NTERM + 1
         DCOEF(NTERM) = - COEF * SQRT (2.0q0) * REAL (IPOWC, KIND = 16)
         WOPER(WMIN,NTERM) = 1
         JOPER(1,NTERM) = K
         JOPER(2,NTERM) = IPOWB
...
!----    Second term of two parts: D^1_0 (J_z^a J_+^b J_-^c + 2 b c J_z^a J_+^{b-1} J_-^{c-1})
      ...
      IF (IPROD > 0) THEN
         NTERM = NTERM + 1
         DCOEF(NTERM) = REAL (IPROD, KIND = 16)
         WOPER(WZER,NTERM) = 1
         JOPER(1,NTERM) = IPOWA
         JOPER(2,NTERM) = IPOWB - 1
         JOPER(3,NTERM) = IPOWC - 1
...
!----    Third term of two parts
      ...
      DO 220 K = 0, IPOWA
         ...
         NTERM = NTERM + 1
         DCOEF(NTERM) = - COEF * SQRT (2.0q0) * REAL (IPOWB, KIND = 16)
         WOPER(WPLU,NTERM) = 1
         JOPER(1,NTERM) = K
         JOPER(2,NTERM) = IPOWB - 1
         JOPER(3,NTERM) = IPOWC
         ...
         IF (IPOWB > 1) THEN
            ...
            DCOEF(NTERM) = - COEF * SQRT (2.0q0) * REAL (IPROD, KIND = 16)
            WOPER(WPLU,NTERM) = 1
            JOPER(1,NTERM) = K
            JOPER(2,NTERM) = IPOWB - 2
            JOPER(3,NTERM) = IPOWC - 1
...
!----    Reduce (Jz - 1)^a to normal form
      ...
      DO 310 K = 0, IPOWA
         ...
         NTERM = NTERM + 1
         DCOEF(NTERM) = COEF * REAL (IPOWC * (IPOWC - 1), KIND = 16)
         WOPER(WMIN,NTERM) = 1
         JOPER(1,NTERM) = K
         JOPER(2,NTERM) = IPOWB
...
!        D^1_0 2^1/2 c (J_z^a J_+^b J_-^{c-1} + b (c-1) J_z^a J_+^{b-1} J_-^{c-2})
      IF (IPOWC > 0) THEN
         NTERM = NTERM + 1
         DCOEF(NTERM) = - SQRT (2.0q0) * REAL (IPOWC, KIND = 16)
         WOPER(WZER,NTERM) = 1
         JOPER(1,NTERM) = IPOWA
         JOPER(2,NTERM) = IPOWB
         JOPER(3,NTERM) = IPOWC - 1
      ...
      IF (IPOWB > 0 .AND. IPOWC > 1) THEN
         ...
         DCOEF(NTERM) = - SQRT (2.0q0) * REAL (IPROD, KIND = 16)
         WOPER(WZER,NTERM) = 1
         JOPER(1,NTERM) = IPOWA
         JOPER(2,NTERM) = IPOWB - 1
         JOPER(3,NTERM) = IPOWC - 2
...
!        D^1_1 2^1/2 c (J_z^a J_+^b J_-^{c-1} + b (c-1) J_z^a J_+^{b-1} J_-^{c-2})
      DO 320 K = 0, IPOWA
         ...
         IF (IPOWB > 0 .AND. IPOWC > 0) THEN
            NTERM = NTERM + 1
            IPROD = 2 * IPOWB * IPOWC
            DCOEF(NTERM) = COEF * REAL (IPROD, KIND = 16)
            WOPER(WPLU,NTERM) = 1
            JOPER(1,NTERM) = K
            JOPER(2,NTERM) = IPOWB - 1
            JOPER(3,NTERM) = IPOWC - 1
...
!---- INPUT:  INT*1 WIGOPER(-1:+1), INT*4 KETROT(2)
!---- OUTPUT: INT*4 KETROTMOD(2,-1:+1), REAL*16 WCOEF
      IMPLICIT REAL (16) (A-H,O-Z), INTEGER (4) (I-N)
...
!----    Function statement
      ...
      IF (N /= 0 .AND. N /= 1) THEN
         PAUSE '#### WIGOPKET #### Exactly one Wigner D(-1,0,+1) operator expected'
         RETURN
      ENDIF
...
!----       Evaluate Wigner 3-j
         ...
         IF (M == -1 .AND. J > 0) THEN
            ...
            ANOM = REAL16 ((J + K - 1) * (J + K))
            DENO = REAL16 (J * (2 * J - 1) * (2 * J + 1))
         ELSE IF (M == 0 .AND. J > 0) THEN
            IZERO = 1
            IF (MOD (ABS(J-K+1),2) > 0) W3J = -W3J
            ANOM = REAL16 ((J - K + 1) * (J + K))
            DENO = REAL16 (J * (J + 1) * (2 * J + 1))
         ELSE IF (M == 1 .AND. J >= 0) THEN
            IZERO = 1
            IF (MOD (ABS(-J-K),2) > 0) W3J = -W3J
            ANOM = REAL16 ((J - K + 1) * (J - K + 2))
            DENO = REAL16 ((J + 1) * (2 * J + 1) * (2 * J + 3))
         ENDIF
         IF (IZERO == 0) THEN
            W3J = 0.0q0
         ELSE
...
!-----------------------------------------------------------------------------------------------
     ...
     &         W3JA(l),W3J,j1,j2,j3,m1,m2,m3,jmin,jmax,l
         ...
         IF (W3J == 0.0q0) CYCLE
         PREFIX = SQRT (REAL16 (2 * J + 1))
         IF (MOD (ABS(K),2) > 0) PREFIX = -PREFIX
         WCOEF(M) = PREFIX * W3J * SQRT (REAL16 (2 * (J + M) + 1))
  110 CONTINUE
      GO TO 900
...
!----          K is outside the square root, check its sign and modify W3J accordingly
            IF (K .LT. 0) W3J = -W3J
            DENO = REAL16 (J * (1 + J) * (1 + 2 * J))
         ELSE IF (M == 1 .AND. J >= 0) THEN
            IZERO = 1
            IF (MOD (ABS(1-J+K),2) > 0) W3J = -W3J
            ANOM = REAL16 ((1 + J - K) * (1 + J + K))
            DENO = REAL16 ((1 + J) * (1 + 2 * J) * (3 + 2 * J))
         ENDIF
         IF (IZERO == 0) THEN
            W3J = 0.0q0
         ELSE
...
!
!  PURPOSE:
!     Calculate the table of binomial coefficients C(n,k) = (n/k), where n >= k >= 0.
!
!  THEORY:
!     The definition of the binomial coefficient b = C(n,k)(n/k): There are b ways to choose 
!     an unordered subset of k elements from a fixed set of n elements.
!     For example, there are (4/2) = 6 ways to choose 2 elements from (1,2,3,4), namely
...
!
!  OUTPUT:
!     STIRL -- Table of the Stirling numbers of the first kind, 
!              available through COMMON /STIRLTAB/.
!     
!-----------------------------------------------------------------------------------------------
