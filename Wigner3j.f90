...
!     http://github.com/Dynetrekk/SHTOOLS
...
!     print*, "Input array is dimensioned ", size(w3j)
	...
	real*16 function a(j)
		integer*4 :: j
		a = (real(j)**2 - real(j2-j3)**2) * (real(j2+j3+1)**2 - real(j)**2) * (real(j)**2-real(m1)**2)
		a = sqrt(a)
	end function a
	...
	real*16 function y(j)
		integer*4 :: j
		y = -real(2*j+1) * &
			( real(m1) * (real(j2)*real(j2+1) - real(j3)*real(j3+1) ) - real(m3-m2)*real(j)*real(j+1) )
	end function y
