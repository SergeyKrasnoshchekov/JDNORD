...
!     the values are rescaled by a factor "scalef" whenever the absolute value of the 3j 
!     coefficient becomes greater than unity. Also, the direction of the iteration starts from 
!     low values of j to high values, but when abs(w3j(j+2)/w3j(j)) is less than one, the 
!     iteration will restart from high to low values. More efficient algorithms might be 
!     found for specific cases (for instance, when all m's are zero).
!     
!     Verification: 
!     
!     The results have been verified against this routine run in quadruple precision.
!     For 1.e7 acceptable random values of j2, j3, m2, and m3 between -200 and 200, the relative
!     error was calculated only for those 3j coefficients that had an absolute value greater 
!     than 1.d-17 (values smaller than this are for all practical purposed zero, and can be 
!     heavily affected by machine roundoff errors or underflow). 853 combinations of parameters 
!     were found to have relative errors greater than 1.d-8. Here I list the minimum value of 
!     max(j2,j3) for different ranges of error, as well as the number of times this occured
!     
...
!     
!     Many times (maybe always), the large relative errors occur when the 3j coefficient changes 
!     sign and is close to zero. (I.e., adjacent values are about 10.e7 times greater in 
!     magnitude.) Thus, if one does not need to know highly accurate values of the 3j 
!     coefficients when they are almost zero (i.e., ~1.d-10) then this routine is probably 
...
!     only calculates one coefficient for each call. I also have no idea if this code was 
!     verified.) Maximum relative errors in this case were less than 1.d-8 for a large number
...
!     		jmin, jmax		Minimum and maximum values out output array.
!
!     Dependencies: None
!     
!     Written by Mark Wieczorek August (2004)
!     
!     August 2009: Based on the suggestions of Roelof Rietbroek, the calculation of RS has been 
!     slightly modified so that division by zero will not cause a run time crash (this behavior 
!     depends on how the compiler treats IEEE floating point exceptions). 
!     These values were never used in the original code when this did occur.
!     
!     Copyright (c) 2005-2009, Mark A. Wieczorek
!     All rights reserved.
!-----------------------------------------------------------------------------------------------
implicit none
integer*4, intent(in)  :: j2, j3, m1, m2, m3
integer*4, intent(out) :: jmin, jmax
real*16, intent(out)   :: w3j(200)
real*16 :: wnmid, wpmid, scalef, denom, rs(j2+j3+1),  &
&   wl(0:j2+j3+1), wu(0:j2+j3+1), xjmin, yjmin, yjmax, zjmax, xj, zj
integer*4 :: j, jnum, jp, jn, k, flag1, flag2, jmid
...
integer*4 function jindex(j)
	integer*4 :: j
	jindex = j-jmin+1
end function jindex

real*16 function a(j)
	integer*4 :: j
	a = (real(j)**2 - real(j2-j3)**2) * (real(j2+j3+1)**2 - real(j)**2) * &
		(real(j)**2-real(m1)**2)
	a = sqrt(a)
end function a
	
real*16 function y(j)
	integer*4 :: j
	y = -real(2*j+1) * ( real(m1) * (real(j2)*real(j2+1) - real(j3)*real(j3+1) ) - &
		real(m3-m2)*real(j)*real(j+1) )
end function y

real*16 function x(j)	
	integer*4 :: j
	x = real(j) * a(j+1)
end function x
	
real*16 function z(j)
	integer*4 :: j
	z = real(j+1)*a(j)
end function z
...
subroutine normw3j
	...
	norm = 0.0d0
	do j = jmin, jmax
		norm = norm + real(2*j+1) * w3j(jindex(j))**2
	enddo
